const PLANS = {
  ...
  album2026: {
    title: 'ALBUM 2026 - DESCARGA INMEDIATA',
    price: 70150,
    success_url: 'https://drive.google.com/drive/folders/1evkK59H3E-IAq_fh97zGnJEaf8FFDsQp',
  },
};
...
export default async function handler(req, res) {
  ...
  const { plan, title, price, success_url } = req.body;
  ...
  if (plan && PLANS[plan]) {
    ...
    planData = {
      ...
      success_url: success_url || FAILURE_URL,
    };
  }
  else {
    return res.status(400).json({ error: 'Parámetros inválidos. Envía "plan" o "title"+"price"' });
  }
